...
SELECT bench_setup_int4_unique(500000, 500000, 0.05);
VACUUM (FREEZE, ANALYZE) inner_unique_int4;
VACUUM (FREEZE, ANALYZE) outer_unique_int4;
CHECKPOINT;

SET enable_hashjoin_alt_table = off;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_unique_int4 o JOIN inner_unique_int4 i USING (k);
SET enable_hashjoin_alt_table = on;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_unique_int4 o JOIN inner_unique_int4 i USING (k);

\echo ''
\echo 'Case 2: int4 unique, missing-heavy, one-batch'
SELECT bench_setup_int4_unique(500000, 500000, 0.95);
VACUUM (FREEZE, ANALYZE) inner_unique_int4;
VACUUM (FREEZE, ANALYZE) outer_unique_int4;
CHECKPOINT;
...
SELECT bench_setup_int4_dup(200000, 4, 500000, 0.05);
VACUUM (FREEZE, ANALYZE) inner_dup_int4;
VACUUM (FREEZE, ANALYZE) outer_dup_int4;
CHECKPOINT;

SET enable_hashjoin_alt_table = off;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_dup_int4 o JOIN inner_dup_int4 i USING (k);
SET enable_hashjoin_alt_table = on;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_dup_int4 o JOIN inner_dup_int4 i USING (k);

\echo ''
\echo 'Case 4: int4 high-duplicate, hit-heavy, one-batch'
SELECT bench_setup_int4_dup(20000, 64, 500000, 0.05);
VACUUM (FREEZE, ANALYZE) inner_dup_int4;
VACUUM (FREEZE, ANALYZE) outer_dup_int4;
CHECKPOINT;
...
SELECT bench_setup_int8_unique(500000, 500000, 0.95);
VACUUM (FREEZE, ANALYZE) inner_unique_int8;
VACUUM (FREEZE, ANALYZE) outer_unique_int8;
CHECKPOINT;

SET enable_hashjoin_alt_table = off;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_unique_int8 o JOIN inner_unique_int8 i USING (k);
SET enable_hashjoin_alt_table = on;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_unique_int8 o JOIN inner_unique_int8 i USING (k);
...
SET enable_hashjoin_alt_table = off;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_unique_text o JOIN inner_unique_text i USING (k);
SET enable_hashjoin_alt_table = on;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_unique_text o JOIN inner_unique_text i USING (k);
...
SET work_mem = '256MB';
SET hash_mem_multiplier = 2;
SELECT bench_setup_int4_skew(500000, 500000, 16, 0.80, 0.05);
VACUUM (FREEZE, ANALYZE) inner_skew_int4;
VACUUM (FREEZE, ANALYZE) outer_skew_int4;
CHECKPOINT;

SET enable_hashjoin_alt_table = off;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_skew_int4 o JOIN inner_skew_int4 i USING (k);
SET enable_hashjoin_alt_table = on;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_skew_int4 o JOIN inner_skew_int4 i USING (k);

\echo ''
\echo 'Case 8: int4 skew, hot-hit-heavy with misses, one-batch'
SELECT bench_setup_int4_skew(500000, 500000, 8, 0.90, 0.30);
VACUUM (FREEZE, ANALYZE) inner_skew_int4;
VACUUM (FREEZE, ANALYZE) outer_skew_int4;
CHECKPOINT;
...
SET enable_hashjoin_alt_table = off;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_skew_text o JOIN inner_skew_text i USING (k);
SET enable_hashjoin_alt_table = on;
EXPLAIN (ANALYZE, BUFFERS, COSTS OFF, TIMING OFF) SELECT count(*) FROM outer_skew_text o JOIN inner_skew_text i USING (k);
